...
app.get('/api/products', async (req, res) => {
  try {
    ...
    const products = await Product.find().populate('images'); // Assuming you have a reference to images in your Product model
    const productsWithImages = products.map(product => {
      const images = product.images.map(image => {
        return {
          ...image._doc,
          images: image.images.map(imgBuffer => imgBuffer.toString('base64'))
        };
      });
    ...
    //   // Assuming there's a field in the Image schema that references the productId
    //   const images = await Image.find({ productId: product.id });

    //   // Combine product data with images
    //   combinedData.push({
    //     ...product.toJSON(), // Sequelize model instance to plain object
    //     images: images.map(image => image.toJSON()) // Convert Mongoose documents to objects
    //   });
    // }
    ...
    res.status(500).json({ error: 'Failed to fetch products and images' });
  }
});
